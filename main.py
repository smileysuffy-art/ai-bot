...
async def auto_checker(app, pair, entry_price, signal_type):
    await asyncio.sleep(125) # 2-min trade completion
            ...
            async with websockets.connect(url) as ws:
                ...
                while True:
                    ...
                    if "symbol" in data and "price" in data:
                        ...
                        if len(price_data[pair]) > 100: price_data[pair].pop(0)
...
async def start(u, c):
    ...
    await u.message.reply_text("💎 Sniper V5 Ultra Ready", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))

async def handle_buttons(u, c):
    global auto_mode
    t = u.message.text
    if t == "▶️ Start": await u.message.reply_text("System Active.")
    elif t == "⛔ Stop": auto_mode = False; await u.message.reply_text("Auto Scan Off.")
    elif t == "📊 Signal": auto_mode = True; await u.message.reply_text("Scanning for Strong Moves... 🔍")

# FIXED: Proper startup task creation
...
def main():
    ...
    # FIXED: post_init callback properly registered
    app = ApplicationBuilder().token(TOKEN).post_init(post_init).build()

    app.add_handler(CommandHandler("start", start))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_buttons))

    print("🔥 Sniper V5 Running...")
