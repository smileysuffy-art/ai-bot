...
# 10 Strong Pairs for Free Tier
PAIRS = ["EUR/USD", "GBP/USD", "USD/JPY", "AUD/USD", "EUR/JPY", "USD/CAD", "USD/CHF", "NZD/USD", "EUR/GBP", "GBP/JPY"]
...
async def auto_checker(app, pair, entry_price, signal_type):
    await asyncio.sleep(125) # 2-min expiry
    ...
    while True:
        try:
            async with websockets.connect(url) as ws:
                logging.info("Starting Batch Subscription for 10 Pairs...")
                # TwelveData Limit Bypass: 3 pairs every 2 seconds
            ...
            logging.error(f"Reconnecting due to error: {e}")
...
async def start(u, c):
    ...
    await u.message.reply_text("💎 Sniper V5 Ultra-Pro Active", reply_markup=ReplyKeyboardMarkup(kb, resize_keyboard=True))
...
async def handle_buttons(u, c):
    ...
    elif t == "📊 Signal": auto_mode = True; await u.message.reply_text("Scanning 10 Pairs... 🔍")
